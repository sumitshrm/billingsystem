...
public class MeasurementSheetController {
    ...
    public String update(@Valid MeasurementSheet measurementSheet, BindingResult bindingResult, Model uiModel, HttpServletRequest httpServletRequest) {
        if (bindingResult.hasErrors()) {
            populateEditForm(uiModel, measurementSheet);
            return "measurementsheets/update";
        }
	...
	void populateEditForm(Model uiModel, MeasurementSheet measurementSheet) {
        uiModel.addAttribute("measurementSheet", measurementSheet);
        addDateTimeFormatPatterns(uiModel);
